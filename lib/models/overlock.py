...
class CTXDownsample(nn.Module):
    # dim channels:256 h_dim channels:512
    def __init__(self, dim, h_dim):
        super().__init__()
        
        self.x_proj = nn.Sequential(
            # 通道double，但是stride=2 降采样，分辨率降低
            nn.Conv2d(dim, h_dim, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(h_dim)
        )
        self.h_proj = nn.Sequential(
            # 通道不变，但是stride=2 降采样，分辨率降低
...
class DynamicConvBlock(nn.Module):
    def __init__(self,
        ...
        self.fusion = nn.Sequential(  #Conv(384 → 384 → 256)
    ...
    def apply_rpb(self, attn, rpb, height, width, kernel_size, idx_h, idx_w, idx_k):
        ...
        return attn + rpb
                    ...
                    #sub_blocks3:x,ctx,ctx_up
    def _forward_inner(self, x, h_x, h_r):  # x:[256, H/16, W/16] h_x:[128, H/16, W/16] h_r:[128, H/16, W/16]
        ...
        x_f = torch.cat([x, h_x], dim=1)  # x_f:[384, H/16, W/16]
        x_f = self.dwconv1(x_f)
        identity = x_f
        x_f = self.norm1(x_f)
        x = self.fusion(x_f)   #sub3:  Conv(384 → 384 → 256)
        gate = self.gate(x)
        lepe = self.lepe(x)
...
class OverLoCK(nn.Module):
    ...
    def get_pyramid_features(self, x):
        """
        专门为分割等下游任务设计的特征提取函数
        返回四个尺度的特征图: [Stage1, Stage2, Stage3, Stage4]
        """
        x = self.patch_embed1(x)
        for blk in self.blocks1:
            x = blk(x)
        ...
        ctx_ori = self.high_level_proj(ctx)
        # ctx_ori:[B, 128, H/32, W/32]
        ctx_up = F.interpolate(ctx_ori, size=x.shape[2:], mode='bilinear', align_corners=False)
        # ctx_up:[B, 128, H/16, W/16]
        for idx, blk in enumerate(self.sub_blocks3):
            if idx == 0:
                ctx = ctx_up
            # ctx:[B, 128, H/16, W/16]
