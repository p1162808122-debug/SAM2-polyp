...
def checkpoint_seq(functions, input, checkpoint_blks, unfold=False):
    """PyTorch 1.11- style gradient checkpointing, also handles older PyTorch."""
    ...
    if checkpoint_blks <= 0:
        return torch.utils.checkpoint.checkpoint(run_function(0, len(functions) - 1, functions), input, use_reentrant=False)
    else:
        if unfold:
            def run_function_checkpointed(start, end, functions):
                def forward(input):
                    for j in range(start, end):
                        input = torch.utils.checkpoint.checkpoint(functions[j], input, use_reentrant=False)
                    input = functions[end](input)
                    return input
                return forward
            return torch.utils.checkpoint.checkpoint(run_function_checkpointed(0, len(functions) - 1, functions), input, use_reentrant=False)
        else:
            checkpoint_kwargs = dict(use_reentrant=False)
            for i in range(1, len(functions)):
                input = torch.utils.checkpoint.checkpoint(run_function(checkpoint_blks * i, checkpoint_blks * i - 1, functions), input, **checkpoint_kwargs)
                for j in range(checkpoint_blks * i, min(checkpoint_blks * (i + 1), len(functions))):
                    input = functions[j](input)
            return input
